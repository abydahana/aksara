...
{
    private string $_table = 'app_groups_privileges';
    ...
    public function index()
    {
        ...
        ->setDescription('
            <div class="row">
                <div class="col-12">
                    ' . phrase('You may review {{group_privileges}} after modifying this adjustment to set new roles.', ['group_privileges' => '<a href="' . go_to('../', ['q' => null, 'page' => null, 'group_id' => null, 'path' => null]) . '" class="badge bg-secondary --xhr"><i class="mdi mdi-arrow-right"></i> ' . phrase('Group Privileges') . '</a>']) . '
                </div>
            </div>
        ')
            ...
            foreach ($privileges as $key => $val) {
                $output .= '
                    <div class="col-md-6 privilege mt-1">
                        ' . (in_array($this->getMethod(), ['create', 'update']) ? '
        ...
        $output = '
            <div class="w-100">
                <div class="row">
                    ...
                    ' . (in_array($this->getMethod(), ['index', 'create', 'update']) ? '
