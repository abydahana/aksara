(function () {
  ...
  const endpoint = window.AksaraAI && window.AksaraAI.endpoint ? window.AksaraAI.endpoint : baseUrl() + 'xhr/ai';
  ...
  const loadingTexts = ['The assistant is reading the form...', 'The assistant is preparing your request...', 'The assistant is arranging the content...', 'The assistant is matching fields carefully...', 'The assistant is almost done...'];
  ...
  function imagePropKeys(type) {
    return ['section', 'container', 'row', 'column'].includes(type) ? [] : 'hero' === type ? ['background'] : ['src', 'image'];
  }
  ...
  function formTitle(form) {
    return document.querySelector('[data-role="title"]')?.textContent.trim() || form.closest('.modal')?.querySelector('.modal-title')?.textContent.trim() || document.title;
  }
      ...
      case 'ol':
        return (
          Array.from(node.children)
            .map((child, index) => listItemToMarkdown(child, tag, index, depth))
            .join('') + '\n'
        );
      case 'li':
        return '- ' + content + '\n';
  ...
  function safeMarkdownHtml(markdown) {
    if (window.marked && window.DOMPurify) {
      ...
      return (
        window.DOMPurify.sanitize(window.marked.parse(markdown || ''), {
          USE_PROFILES: {
            html: true
          }
  ...
  function normalizeAiResponse(response) {
    ...
    try {
      ...
      if (content && typeof content === 'object' && !Array.isArray(content)) {
        normalized.fields = looksLikePageBuilderLayout(content) ? { page_content: JSON.stringify(content) } : content;
      }
    } catch (error) {
      normalized.fields = {};
    }
  ...
  function pageBuilderPreviewHtml(value) {
    try {
      ...
      if (window._pageBuilder && typeof window._pageBuilder.renderBlocks === 'function' && components.length) {
        preview = '<div class="border rounded bg-body-tertiary p-2 mt-2" style="max-height:420px;overflow:auto" data-ai-pagebuilder-preview>' + window._pageBuilder.renderBlocks(components) + '</div>';
      }
  ...
  function formFromButton(button) {
    return button.closest('form') || button.closest('.modal, .container-fluid, article')?.querySelector('form.--validate-form') || document.querySelector('form.--validate-form');
  }
