...
(function() {
      ...
      root._sortable = new Sortable(root, {
        ...
        onAdd: function(event) {
          ...
          if (itemElement.classList.contains('pb-component-item')) {
            ...
            component = self.wrapComponentForRoot(type);
  ...
  Prototype.renderContent = function(component) {
    ...
    switch (component.type) {
      ...
      case 'accordion':
        ...
        return accHtml;
      case 'carousel':
        let carHtml = '<div class="bg-body-tertiary text-body rounded p-3 text-center"><i class="mdi mdi-view-carousel-outline mdi-2x"></i><strong>' + phrase('Carousel Slider') + '</strong><div class="mt-2 small">';
        (props.items || []).forEach(function(item, i) {
          carHtml += '<span class="badge bg-secondary me-1">' + (i + 1) + '</span>';
        });
        return carHtml + '</div></div>';
      case 'tabs':
        let tabHtml = '<div class="bg-body-tertiary text-body rounded p-2"><strong>' + phrase('Tabs') + ' (' + phrase(props.alignment || 'horizontal') + ')</strong><div class="d-flex gap-1 mt-1">';
        (props.items || []).forEach(item => {
      ...
      case 'pricing':
        ...
        return '<div class="border rounded p-3 text-center bg-body-tertiary text-body' + priceFeatured + '"><h5 class="fw-bold mb-1">' + phrase(props.title || 'Plan') + '</h5><h4 class="mb-0">' + (props.price || '$0') + '</h4><small class="text-muted d-block mb-2">' + phrase(props.period || '') + '</small><span class="btn btn-primary btn-sm rounded-pill">' + phrase(props.btn_text || 'Buy Now') + '</span></div>';
      case 'testimonial':
        ...
        return '<div class="bg-body-tertiary text-body rounded p-3 italic">' + phrase(this.markdownToHtml(props.quote) || '...') + '<div class="mt-2 d-flex align-items-center">' + testImg + '<strong>' + phrase(props.author || '') + '</strong></div></div>';
      case 'team_member':
   ...
   * Whitelist: <strong>, <b>, <i>, <u>, <del>, <s>, <strike>, <strikethrough>, <a>, <ol>, <ul>, <li>, <p>, <br>
   */
    ...
    if (options === 'colorpicker') {
      ...
      html +=
        '<div class="input-group input-group-sm">' +
          '<button type="button" class="btn btn-secondary pb-icon-picker-btn" data-role="iconpicker" data-iconset="materialdesign" data-icon="' + (value || 'mdi mdi-radiobox-blank') + '" data-key="' + key + '"></button>' +
  ...
  Prototype.openImageBrowser = function(targetInput) {
    ...
    const loadImages = function() {
      ...
      const url = self.withQuery((self.options.images_url || 'builderImages'), {
        q: state.q,
        sort: state.sort,
        page: state.page
      });
      ...
      fetch(url).then(res => res.json()).then(data => {
          ...
          item.onclick = function(e) {
            ...
            if (delBtn) {
              ...
              if (confirm(phrase('Are you sure you want to delete this image?'))) {
                ...
                fetch(self.withQuery((self.options.delete_url || 'builder-delete')), { method: 'POST', body: formData })
                  .then(res => res.json())
                  .then(res => {
        ...
        if (data.totalPages > 1) {
          ...
          for (let i = 1; i <= data.totalPages; i++) {
            if (i === 1 || i === data.totalPages || (i >= state.page - 1 && i <= state.page + 1)) {
          ...
          ph += '<div class="pb-page-btn ' + (state.page === data.totalPages ? 'disabled' : '') + '" data-page="' + (state.page + 1) + '"><i class="mdi mdi-chevron-right"></i></div>';
    ...
    fileInput.onchange = function() {
      ...
      fetch(self.withQuery((self.options.upload_url || 'builderUpload')), { method: 'POST', body: formData })
        .then(res => res.json())
        .then(data => {
  ...
  Prototype.initializeWysiwyg = function(component) {
    ...
    this.propertiesElement.querySelectorAll('.pb-wysiwyg').forEach(function(container) {
      ...
      if (cmd === 'createLink') {
            ...
            '<div class="modal-dialog modal-sm modal-dialog-centered">' +
                '<div class="modal-content">' +
                    ...
                    '<div class="modal-body">' +
                        '<div class="mb-3">' +
                            '<label class="form-label">' + phrase('Link Label') + '</label>' +
                            '<input type="text" class="form-control" id="pb-link-label" value="' + selectedText + '" placeholder="' + phrase('Text to display') + '">' +
                        '</div>' +
                        '<div class="mb-3">' +
                            '<label class="form-label">' + phrase('URL') + '</label>' +
                            '<input type="text" class="form-control" id="pb-link-url" value="' + existingUrl + '" placeholder="https://">' +
                        '</div>' +
                    '</div>' +
        ...
        modalEl.querySelector('.pb-link-save').addEventListener('click', function() {
          ...
          html +=
            '<div class="pb-wysiwyg mt-1" data-key="' + key + '" data-item-idx="' + index + '">' +
              '<div class="pb-wysiwyg-toolbar">' +
                ...
                '<button type="button" data-cmd="removeFormat" data-bs-toggle="tooltip" title="' + phrase('Clear Formatting') + '"><i class="mdi mdi-format-clear"></i></button>' +
              '</div>' +
              '<div class="pb-wysiwyg-editor bg-body text-body" contenteditable="true" style="min-height:120px;max-height:300px;overflow-y:auto;padding:12px 14px;font-size:14px;line-height:1.4;outline:none">' + (this.markdownToHtml(item[key]) || '') + '</div>' +
            '</div>';
        } else {
