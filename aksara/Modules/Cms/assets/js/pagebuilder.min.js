...
(function() {
  ...
  Prototype.renderContent = function(component) {
    ...
    switch (component.type) {
      ...
      case 'divider':
        return '<hr/>';
      case 'button':
        return '<span class="btn btn-' + (props.style || 'primary') + ' btn-sm' + (props.rounded ? ' rounded-pill' : '') + '" style="pointer-events:none">' + phrase(props.text || 'Button') + '</span>';
      case 'image':
      ...
      case 'card':
        return '<div class="border rounded p-2"><strong>' + phrase(props.title || 'Card Title') + '</strong><br><small>' + phrase(props.text || '') + '</small></div>';
      case 'spacer':
      ...
      case 'tabs':
        ...
        (props.items || []).forEach(item => {
          tabHtml += '<div class="border rounded px-2 py-1 small bg-white">' + phrase(item.title) + '</div>';
        });
        return tabHtml + '</div></div>';
      case 'pricing':
        const priceFeatured = props.featured ? ' border-primary' : '';
        return '<div class="border rounded p-3 text-center' + priceFeatured + '"><h5 class="fw-bold mb-1">' + phrase(props.title || 'Plan') + '</h5><h4 class="mb-0">' + (props.price || '$0') + '</h4><small class="text-muted d-block mb-2">' + phrase(props.period || '') + '</small><span class="btn btn-primary btn-sm rounded-pill">' + phrase(props.btn_text || 'Buy Now') + '</span></div>';
      case 'testimonial':
        return '<div class="bg-light rounded p-3 italic">"' + phrase(this.markdownToHtml(props.quote) || '...') + '"<div class="mt-2 d-flex align-items-center"><div class="rounded-circle bg-secondary me-2" style="width:30px;height:30px"></div><strong>' + phrase(props.author || '') + '</strong></div></div>';
      case 'team_member':
        return '<div class="text-center"><div class="rounded-circle bg-secondary mx-auto mb-2" style="width:60px;height:60px"></div><strong>' + phrase(props.name || '') + '</strong><br><small class="text-primary">' + phrase(props.role || '') + '</small></div>';
      case 'cta':
        ...
        return '<div class="d-flex justify-content-between align-items-center py-4 px-5 rounded-4 text-start" style="background:' + ctaBg + ';color:' + ctaColor + ';min-height:120px">' +
  ...
  Prototype.renderProperties = function() {
    ...
    grouping.forEach(group => {
      ...
      group.forEach(key => {
        if (key in defaults) {
          ...
          html += '<div class="col">' + this.renderPropertyField(key, value, options, true) + '</div>';
    ...
    for (const key in defaults) {
      ...
      const value = props[key] !== undefined ? props[key] : defaults[key];
      const options = definition.options ? definition.options[key] : null;
      html += this.renderPropertyField(key, value, options);
    }
   ...
   */
  Prototype.renderPropertyField = function(key, value, options, isInsideRow) {
    ...
    } else if (['text', 'subtitle', 'body', 'bio', 'quote'].indexOf(key) > -1) {
      html += 
  ...
  Prototype.bindPropertyEvents = function(component) {
    const self = this;
    this.propertiesElement.querySelectorAll('.pb-property-input').forEach(function(input) {
      ...
      else input.addEventListener('input', updateValue);
    });
