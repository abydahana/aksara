...
<div class="<?= 'modal' !== service('request')->getPost('prefer') ? 'container-fluid py-3' : '' ?>">
    <div class="position-relative ratio ratio-4x3 rounded-4 bg-dark overflow-hidden mb-3">
    ...
    <h5>
        <?= $detail->name ?>
        <?= 'backend' == $detail->type
            ? '<span class="badge bg-dark float-end rounded-pill">' . phrase('Backend Theme') . '</span>'
            : ('frontend' == $detail->type
            ? '<span class="badge bg-success float-end rounded-pill">' . phrase('Frontend Theme') . '</span>'
            : '<span class="badge bg-primary float-end rounded-pill">' . phrase('Module') . '</span>') ?>
    </h5>
    <div class="mb-3">
    ...
    <?php if (isset($detail->reviews) && $detail->reviews): ?>
        <hr class="my-3" style="margin-inline: -1rem" />
        <div class="mb-0">
            <b class="d-block mb-3"><?= phrase('User Reviews') ?></b>
            <div class="pe-1">
                <?php foreach ($detail->reviews as $key => $review): ?>
                    <?= $key > 0 ? '<hr class="my-3" style="margin-inline: -1rem" />' : '' ?>
                    <div class="mb-3 pb-2">
