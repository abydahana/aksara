...
class Document
{
    ...
    private function _word($html = null, $filename = null, $method = 'embed', $params = [])
    {
        ...
        $pageWidthTWIP = $this->_convertToTwip($widthMatches[1] ?? 0, $widthMatches[2] ?? '', $MM_TO_TWIP_VALUE);
        $pageHeightTWIP = $this->_convertToTwip($heightMatches[1] ?? 0, $heightMatches[2] ?? '', $MM_TO_TWIP_VALUE);
