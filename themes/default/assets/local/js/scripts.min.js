...
$(document).ready(function () {
  ...
  function openSubSheet(title, items) {
    ...
    items.forEach(function (item) {
      ...
      listHtml += '<li>';
      listHtml += '<a href="' + (item.hasChildren ? 'javascript:void(0)' : item.href) + '"';
        ...
        if (Array.isArray(data) && data.length) {
          data.forEach(function (item) {
            if (item.language && item.code) {
              ...
              listHtml += '<img src="' + (item.avatar || '') + '" style="width:36px;height:36px;border-radius:50%;object-fit:cover;flex-shrink:0" alt="' + (item.user || '') + '" onerror="this.style.display=\'none\'" />';
