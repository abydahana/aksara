...
$(document).ready(function() {
  ...
  $('body').on('click', '[data-toggle=theme]', function(e) {
    ...
    $.ajax({
      url: (typeof config !== 'undefined' && config.base_url ? config.base_url : '/') + 'xhr/theme_toggle',
      method: 'POST',
      data: {
        theme: next
      }
...
$(document).ready(function () {
  ...
  function openSubSheet(title, items) {
    ...
    $sheet.find('[data-bs-submenu]').on('click', function (e) {
      ...
      var itemIndex = 0, nonDividerCount = 0;
    ...
    $.ajax({
        ...
        if (Array.isArray(data) && data.length) {
          data.forEach(function (item) {
            if (item.language && item.code) {
              /* Language item */
              var href = config.base_url + 'xhr/language/' + item.code;
              ...
              listHtml += '<img src="' + (item.avatar || '') + '" style="width:36px;height:36px;border-radius:50%;object-fit:cover;flex-shrink:0" alt="' + (item.user || '') + '" loading="lazy" decoding="async" onerror="this.style.display=\'none\'" />';
              ...
              if (item.timestamp) {
                listHtml += '<span style="font-size:0.75rem;color:var(--bs-sheet-accent)">' + item.timestamp + '</span>';
              }
              listHtml += '</span>';
              listHtml += '</a>';
              listHtml += '</li>';
      ...
      error: function (xhr) {
        ...
        try {
          ...
          if (!found) {
            $sheet.find('.sub-sheet-body').html(
              '<div style="padding:12px 20px">' + html + '</div>'
            );
          }
        } catch (e) {
          $sheet.find('.sub-sheet-body').html(
            '<div style="text-align:center;padding:32px 16px;color:var(--bs-sheet-text-muted)">' +
              ...
              '<p style="margin-top:8px">Failed to load content</p>' +
            '</div>'
          );
        }
      }
