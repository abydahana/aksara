...
Our team uses [PHP CS Fixer](https://github.com/FriendsOfPHP/PHP-CS-Fixer) to apply coding standard fixes automatically.
...
- Methods SHOULD be ordered by visibility so the public API is easy to read first: `public` methods first, followed by `protected` methods, and `private` methods last.
- Private methods SHOULD be placed at the end of the class because they are implementation details that support the public behavior.
...
- Files containing templates SHOULD end with a ".twig" extension.
...
- Files declaring helper or global functions SHOULD be written in `snake_case` format.
...
- Methods and properties declared inside classes MUST be written in `camelCase`.
- Helper and global function names MUST be written in `snake_case`.
