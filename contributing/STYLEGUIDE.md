...
- Method and property names MUST be declared in `snake_case`.
...
```diff
-$this->assert_equals(12, (int) $axis);
+$this->assert_same(12, (int) $axis);
```

- Use the dedicated assertion instead of using internal types.
```diff
-$this->assert_same(true, is_cli());
+$this->assert_true(is_cli());

-$this->assert_true(array_key_exists('foo', $array));
+$this->assert_array_has_key('foo', $array);
```