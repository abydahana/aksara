`havingLike()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`havingLike()` menambahkan kondisi AND HAVING LIKE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`havingLike(string|array $field = [], mixed $match = '', string $side = 'both', bool $escape = true, bool $caseInsensitive = false): static`
...
### Perilaku
`havingLike()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->havingLike('customer_name', 'Aby');
