`whereIn()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`whereIn()` menambahkan kondisi AND WHERE IN. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`whereIn(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Perilaku
`whereIn()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->whereIn('orders.status', ['lunas', 'settled']);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->whereIn('orders.status', ['lunas', 'settled']);
