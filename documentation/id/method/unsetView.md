`unsetView()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`unsetView()` menyembunyikan field dari read view. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`unsetView(string|array $params = []): static`
...
### Perilaku
`unsetView()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->unsetView('internal_notes');
