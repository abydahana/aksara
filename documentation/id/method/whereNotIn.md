`whereNotIn()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`whereNotIn()` menambahkan kondisi AND WHERE NOT IN. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`whereNotIn(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Perilaku
`whereNotIn()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->whereNotIn('orders.status', ['batal', 'arsip']);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->whereNotIn('orders.status', ['batal', 'arsip']);
