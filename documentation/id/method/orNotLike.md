`orNotLike()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orNotLike()` menambahkan kondisi OR NOT LIKE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`orNotLike(string|array $field = [], mixed $match = '', string $side = 'both', bool $escape = true, bool $caseInsensitive = false): static`
...
### Perilaku
`orNotLike()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->orNotLike('orders.customer_name', 'demo');
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orNotLike('orders.customer_name', 'demo');
