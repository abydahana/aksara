`addDropdown()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`addDropdown()` menambahkan action dropdown per baris di tabel. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`addDropdown(string $url, string $label, ?string $class = null, ?string $icon = null, ?array $parameter = [], bool $newTab = false, ?string $attribution = null): static`
...
### Perilaku
`addDropdown()` menyimpan konfigurasi interface pada controller. Renderer aktif membacanya untuk tombol, filter, heading, layout, variable theme, atau payload output.

### Contoh Dasar
```php
$this->addDropdown('pesanan/receipt', phrase('Struk'), 'btn btn-sm btn-outline-secondary', 'mdi mdi-receipt', ['order_id' => 'order_id']);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->addDropdown('pesanan/receipt', phrase('Struk'), 'btn btn-sm btn-outline-secondary', 'mdi mdi-receipt', ['order_id' => 'order_id']);
