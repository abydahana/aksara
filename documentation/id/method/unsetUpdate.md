`unsetUpdate()` menonaktifkan update untuk record dengan nilai primary tertentu.

### Tujuan
`unsetUpdate()` menonaktifkan update untuk record dengan nilai primary tertentu. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`unsetUpdate(string|array $params = [], array $value = []): static`

### Parameter
| Parameter | Tipe | Wajib | Default | Keterangan |
|---|---|---:|---|---|
| `$params` | <code>string&#124;array</code> | Tidak | `[]` | Nilai, daftar nilai, atau pasangan key/value yang diterima metode ini. |
| `$value` | `array` | Tidak | `[]` | Nilai untuk field, option, kondisi, atau kontrol yang dibuat. |
...
### Perilaku
`unsetUpdate()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->unsetUpdate('status', ['lunas']);
