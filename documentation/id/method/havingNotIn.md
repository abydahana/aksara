`havingNotIn()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`havingNotIn()` menambahkan kondisi AND HAVING NOT IN. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`havingNotIn(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Perilaku
`havingNotIn()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->havingNotIn('orders.status', ['batal']);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->havingNotIn('orders.status', ['batal']);
