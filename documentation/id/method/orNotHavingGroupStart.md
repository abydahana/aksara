`orNotHavingGroupStart()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orNotHavingGroupStart()` membuka grup kondisi OR NOT HAVING. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Perilaku
`orNotHavingGroupStart()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query. Panggilan pembuka dan penutup grup harus seimbang.

### Contoh Dasar
```php
$this->orNotHavingGroupStart()->having('total_amount <', 1000)->havingGroupEnd();
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orNotHavingGroupStart()->having('total_amount <', 1000)->havingGroupEnd();
