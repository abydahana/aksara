`where()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`where()` menambahkan kondisi AND WHERE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`where(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Contoh Dasar
```php
$this->where('orders.status', 'lunas');
