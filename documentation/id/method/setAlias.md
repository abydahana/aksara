`setAlias()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`setAlias()` mengatur label field yang ramah dibaca pengguna. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`setAlias(string|array $params = [], ?string $value = null): static`
...
### Perilaku
`setAlias()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->setAlias('order_number', phrase('Nomor Pesanan'));
