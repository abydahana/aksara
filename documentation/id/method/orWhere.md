`orWhere()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orWhere()` menambahkan kondisi OR WHERE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`orWhere(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Perilaku
`orWhere()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.
