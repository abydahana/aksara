`setDescription()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`setDescription()` mengatur deskripsi halaman dan metadata response. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`setDescription(array|string $params = [], ?string $fallback = null): static`
...
### Perilaku
`setDescription()` menyimpan konfigurasi interface pada controller. Renderer aktif membacanya untuk tombol, filter, heading, layout, variable theme, atau payload output.

### Contoh Dasar
```php
$this->setDescription(phrase('Kelola pesanan pelanggan.'));
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->setDescription(phrase('Kelola pesanan pelanggan.'));
