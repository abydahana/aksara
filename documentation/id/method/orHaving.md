`orHaving()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orHaving()` menambahkan kondisi OR HAVING. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`orHaving(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Perilaku
`orHaving()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->orHaving('total_item >', 5);
