`fieldOrder()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`fieldOrder()` mengatur urutan field pada form. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`fieldOrder(string|array $params = []): static`
...
### Perilaku
`fieldOrder()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->fieldOrder('order_number, customer_id, status, notes');
