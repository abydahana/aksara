`orGroupStart()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orGroupStart()` membuka grup kondisi OR WHERE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Perilaku
`orGroupStart()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query. Panggilan pembuka dan penutup grup harus seimbang.

### Contoh Dasar
```php
$this->orGroupStart()->where('orders.status', 'lunas')->where('orders.priority', 'high')->groupEnd();
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orGroupStart()->where('orders.status', 'lunas')->where('orders.priority', 'high')->groupEnd();
