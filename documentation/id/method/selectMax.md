...
### Referensi
`selectMax(string $column, ?string $alias = null): static`
...
### Perilaku
`selectMax()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->selectMax('orders.amount', 'amount_terbesar');
...
class Pesanan extends Core
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->selectMax('orders.amount', 'amount_terbesar');
