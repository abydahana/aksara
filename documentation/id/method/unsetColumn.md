`unsetColumn()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`unsetColumn()` menyembunyikan field dari tampilan tabel. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`unsetColumn(string|array $params = []): static`
...
### Perilaku
`unsetColumn()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->unsetColumn('internal_notes');
