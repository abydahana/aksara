`groupEnd()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`groupEnd()` menutup grup kondisi WHERE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Perilaku
`groupEnd()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query. Panggilan pembuka dan penutup grup harus seimbang.
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->groupStart()->where('orders.status', 'lunas')->orWhere('orders.status', 'pending')->groupEnd();
