`fieldAppend()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`fieldAppend()` menambahkan konten setelah input field. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`fieldAppend(string|array $params = [], ?string $value = null): static`
...
### Perilaku
`fieldAppend()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->fieldAppend('amount', 'IDR');
