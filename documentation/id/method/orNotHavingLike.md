`orNotHavingLike()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orNotHavingLike()` menambahkan kondisi OR HAVING NOT LIKE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`orNotHavingLike(string|array $field = [], mixed $match = '', string $side = 'both', bool $escape = true, bool $caseInsensitive = false): static`
...
### Perilaku
`orNotHavingLike()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->orNotHavingLike('customer_name', 'Test');
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orNotHavingLike('customer_name', 'Test');
