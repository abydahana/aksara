`having()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`having()` menambahkan kondisi AND HAVING. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`having(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Contoh Dasar
```php
$this->having('total_amount >', 100000);
