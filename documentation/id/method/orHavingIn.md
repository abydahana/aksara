`orHavingIn()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orHavingIn()` menambahkan kondisi OR HAVING IN. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`orHavingIn(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Perilaku
`orHavingIn()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->orHavingIn('orders.status', ['pending', 'review']);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orHavingIn('orders.status', ['pending', 'review']);
