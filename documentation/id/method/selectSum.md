`selectSum()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`selectSum()` menambahkan agregasi SUM ke daftar SELECT. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`selectSum(string $column, ?string $alias = null): static`
...
### Perilaku
`selectSum()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->selectSum('orders.amount', 'total_amount');
