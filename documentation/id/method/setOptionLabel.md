`setOptionLabel()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`setOptionLabel()` mengatur label tampilan untuk nilai option. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`setOptionLabel(string|array $params = [], ?string $value = null): static`
...
### Perilaku
`setOptionLabel()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->setOptionLabel('status', ['lunas' => phrase('Lunas'), 'pending' => phrase('Pending')]);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->setOptionLabel('status', ['lunas' => phrase('Lunas'), 'pending' => phrase('Pending')]);
