`addButton()` menambahkan tombol aksi per baris di tabel.

### Tujuan
`addButton()` menambahkan tombol aksi per baris di tabel. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Perilaku
`addButton()` menyimpan konfigurasi antarmuka pada controller. Renderer aktif membacanya untuk tombol, filter, heading, layout, variable theme, atau payload output.
