`addButton()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`addButton()` menambahkan tombol action per baris di tabel. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`addButton(string $url, string $label, ?string $class = null, ?string $icon = null, ?array $parameter = [], bool $newTab = false, ?string $attribution = null): static`
...
### Perilaku
`addButton()` menyimpan konfigurasi interface pada controller. Renderer aktif membacanya untuk tombol, filter, heading, layout, variable theme, atau payload output.

### Contoh Dasar
```php
$this->addButton('pesanan/read', phrase('Lihat'), 'btn btn-sm btn-outline-secondary', 'mdi mdi-eye', ['order_id' => 'order_id']);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->addButton('pesanan/read', phrase('Lihat'), 'btn btn-sm btn-outline-secondary', 'mdi mdi-eye', ['order_id' => 'order_id']);
