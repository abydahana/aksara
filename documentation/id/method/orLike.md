`orLike()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orLike()` menambahkan kondisi OR LIKE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`orLike(string|array $field = [], mixed $match = '', string $side = 'both', bool $escape = true, bool $caseInsensitive = false): static`
...
### Perilaku
`orLike()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->orLike('orders.customer_name', 'Aby');
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orLike('orders.customer_name', 'Aby');
