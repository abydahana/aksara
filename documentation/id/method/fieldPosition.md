`fieldPosition()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`fieldPosition()` memindahkan field ke posisi layout tertentu. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`fieldPosition(string|array $params = [], ?string $value = null): static`
...
### Perilaku
`fieldPosition()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->fieldPosition('notes', 'bottom');
