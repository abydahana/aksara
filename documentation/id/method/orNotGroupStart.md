`orNotGroupStart()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orNotGroupStart()` membuka grup kondisi OR NOT WHERE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Perilaku
`orNotGroupStart()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query. Panggilan pembuka dan penutup grup harus seimbang.

### Contoh Dasar
```php
$this->orNotGroupStart()->where('orders.status', 'arsip')->groupEnd();
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orNotGroupStart()->where('orders.status', 'arsip')->groupEnd();
