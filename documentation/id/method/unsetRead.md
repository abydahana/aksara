`unsetRead()` menonaktifkan read untuk record dengan nilai primary tertentu.

### Tujuan
`unsetRead()` menonaktifkan read untuk record dengan nilai primary tertentu. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`unsetRead(string|array $params = [], array $value = []): static`
...
### Perilaku
`unsetRead()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->unsetRead('status', ['arsip']);
