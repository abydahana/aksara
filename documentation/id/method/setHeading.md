`setHeading()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`setHeading()` mengatur heading untuk grup read atau form. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`setHeading(string|array $params = [], ?string $value = null): static`
...
### Perilaku
`setHeading()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->setHeading('customer_name', phrase('Informasi Pelanggan'));
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->setHeading('customer_name', phrase('Informasi Pelanggan'));
