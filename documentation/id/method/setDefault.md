`setDefault()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`setDefault()` mengatur nilai tetap yang ditulis saat create atau update. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`setDefault(string|array $params = [], mixed $value = null): static`
...
### Contoh Dasar
```php
$this->setDefault('created_by', get_userdata('user_id'));
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->setDefault('created_by', get_userdata('user_id'));
