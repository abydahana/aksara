`setValidation()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`setValidation()` mengatur rule validasi untuk field form. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`setValidation(string|array $params = [], ?string $value = null): static`
