`fieldPrepend()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`fieldPrepend()` menambahkan konten sebelum input field. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`fieldPrepend(string|array $params = [], ?string $value = null): static`
...
### Perilaku
`fieldPrepend()` memperbarui metadata field yang dibaca renderer tabel, read, form, API, dan dokumen. Efeknya muncul saat `render()` melakukan serialisasi dan menyiapkan response.

### Contoh Dasar
```php
$this->fieldPrepend('amount', 'Rp');
