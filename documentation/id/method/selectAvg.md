`selectAvg()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`selectAvg()` menambahkan agregasi AVG ke daftar SELECT. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`selectAvg(string $column, ?string $alias = null): static`
...
### Perilaku
`selectAvg()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->selectAvg('orders.amount', 'rata_rata_amount');
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->selectAvg('orders.amount', 'rata_rata_amount');
