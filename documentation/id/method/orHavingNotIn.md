`orHavingNotIn()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orHavingNotIn()` menambahkan kondisi OR HAVING NOT IN. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`orHavingNotIn(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Perilaku
`orHavingNotIn()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->orHavingNotIn('orders.status', ['arsip']);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orHavingNotIn('orders.status', ['arsip']);
