`notHavingLike()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`notHavingLike()` menambahkan kondisi AND HAVING NOT LIKE. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`notHavingLike(string|array $field = [], mixed $match = '', string $side = 'both', bool $escape = true, bool $caseInsensitive = false): static`
...
### Perilaku
`notHavingLike()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->notHavingLike('customer_name', 'Demo');
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->notHavingLike('customer_name', 'Demo');
