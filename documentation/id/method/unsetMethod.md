`unsetMethod()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`unsetMethod()` menonaktifkan satu atau beberapa action Core. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`unsetMethod(array|string $params = []): static`
...
### Perilaku
`unsetMethod()` menyimpan konfigurasi interface pada controller. Renderer aktif membacanya untuk tombol, filter, heading, layout, variable theme, atau payload output.

### Contoh Dasar
```php
$this->unsetMethod('delete');
