`allowTokenFrom()` mengizinkan route ini menerima token yang dibuat untuk URI lain.

### Tujuan
`allowTokenFrom()` mengizinkan route ini menerima token yang dibuat untuk URI lain. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`allowTokenFrom(string|array $uris = []): static`
...
### Contoh Dasar
```php
$this->allowTokenFrom('pesanan/create');
