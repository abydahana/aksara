`orWhereIn()` adalah Core method yang tersedia di dalam controller Aksara.

### Tujuan
`orWhereIn()` menambahkan kondisi OR WHERE IN. Metode ini menjaga kustomisasi modul tetap berada di alur controller Core bawaan.
...
### Referensi
`orWhereIn(string|array $field = [], mixed $value = '', bool $escape = true): static`
...
### Perilaku
`orWhereIn()` menyimpan instruksi query di state persiapan Core. Instruksi diterapkan saat `render()` membangun query akhir; pemanggilan metode ini saja tidak mengeksekusi query.

### Contoh Dasar
```php
$this->orWhereIn('orders.status', ['pending', 'review']);
...
class Pesanan extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Pesanan'))
            ->orWhereIn('orders.status', ['pending', 'review']);
