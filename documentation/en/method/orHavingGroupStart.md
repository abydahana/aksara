...
### Behavior
`orHavingGroupStart()` records a query instruction on the controller. Core applies that instruction when `render()` compiles the final query. It does not execute a database query by itself.

### Basic Usage
```php
$this->having('region', 'Asia')->orHavingGroupStart()->having('total_orders >', 10)->having('grand_total >', 1000000)->havingGroupEnd();
...
class Orders extends Core
{
    public function index()
    {
        $this->setTitle(phrase('Orders'))
            ->having('region', 'Asia')->orHavingGroupStart()->having('total_orders >', 10)->having('grand_total >', 1000000)->havingGroupEnd();
