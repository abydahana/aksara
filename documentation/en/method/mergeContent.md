...
### Reference
`mergeContent(string $magicString, ?string $alias = null, ?string $callback = null)`
...
### Behavior
`mergeContent()` updates field metadata used by the renderer. The generated output changes when the table, read, or form view is rendered.

### Basic Usage
```php
$this->mergeContent('{{ first_name }} {{ last_name }}', phrase('Full Name'));
