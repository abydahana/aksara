...
### Behavior
`groupEnd()` records a query instruction on the controller. Core applies that instruction when `render()` compiles the final query. It does not execute a database query by itself.

### Basic Usage
```php
$this->groupStart()->where('status', 'paid')->orWhere('status', 'processing')->groupEnd();
...
class Orders extends Core
{
    public function index()
    {
        $this->setTitle(phrase('Orders'))
            ->groupStart()->where('status', 'paid')->orWhere('status', 'processing')->groupEnd();
