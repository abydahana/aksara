...
|---|---|---:|---|---|
| `$flag` | `bool` | No | `true` | Method parameter. |
...
### Behavior
`distinct()` records a query instruction on the controller. Core applies that instruction when `render()` compiles the final query. It does not execute a database query by itself.
