`like()` adds AND LIKE conditions. It is used inside an Aksara controller as part of the Core method API.

### Purpose
`like()` adds AND LIKE conditions. It lets a controller customize Aksara Core behavior while keeping the request inside the built-in CRUD, rendering, permission, validation, and response pipeline.
...
### Reference
`like(string|array $field = [], mixed $match = '', string $side = 'both', bool $escape = true, bool $caseInsensitive = true)`
...
| `$caseInsensitive` | `bool` | No | `true` | Use case-insensitive matching when supported. |
...
### Basic Usage
```php
$this->like('customers.customer_name', $this->request->getGet('q'));
...
class Orders extends Core
{
    public function index()
    {
        $this->setTitle(phrase('Orders'))
            ->like('customers.customer_name', $this->request->getGet('q'));
