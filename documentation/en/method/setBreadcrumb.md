...
### Reference
`setBreadcrumb(array|string $params = [], ?string $value = null)`
...
### Behavior
`setBreadcrumb()` stores request-level configuration on the controller. Call it before the permission, rendering, or form-processing step that depends on it.

### Basic Usage
```php
$this->setBreadcrumb('orders', phrase('Orders'));
...
class Orders extends Core
{
    ...
    {
        $this->setTitle(phrase('Orders'))
            ->setBreadcrumb('orders', phrase('Orders'));
