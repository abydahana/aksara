...
### Reference
`selectSubquery(object|string $subquery, string $alias)`
...
### Behavior
`selectSubquery()` records a query instruction on the controller. Core applies that instruction when `render()` compiles the final query. It does not execute a database query by itself.

### Basic Usage
```php
$paidOrders = $this->model->builder('orders')
    ->select('COUNT(*)')
    ->where('status', 'paid');

$this->selectSubquery($paidOrders, 'paid_orders');
...
class Orders extends Core
{
    public function index()
    {
        ...
        $this->setTitle(phrase('Orders'))
            ->selectSubquery($paidOrders, 'paid_orders');
