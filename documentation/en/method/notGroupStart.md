...
### Behavior
`notGroupStart()` records a query instruction on the controller. Core applies that instruction when `render()` compiles the final query. It does not execute a database query by itself.

### Basic Usage
```php
$this->notGroupStart()->where('status', 'cancelled')->orWhere('status', 'archived')->groupEnd();
...
class Orders extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Orders'))
            ->notGroupStart()->where('status', 'cancelled')->orWhere('status', 'archived')->groupEnd();
