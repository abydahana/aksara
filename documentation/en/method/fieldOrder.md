...
### Reference
`fieldOrder(string|array $params = [])`
...
### Behavior
`fieldOrder()` updates field metadata used by the renderer. The generated output changes when the table, read, or form view is rendered.

### Basic Usage
```php
$this->fieldOrder('title, slug, category_id, content, status');
...
class Orders extends Core
{
    public function index()
    {
        $this->setTitle(phrase('Orders'))
            ->fieldOrder('title, slug, category_id, content, status');
