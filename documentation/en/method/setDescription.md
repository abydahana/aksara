`setDescription()` sets the page description. It is used inside an Aksara controller as part of the Core method API.

### Purpose
`setDescription()` sets the page description. It lets a controller customize Aksara Core behavior while keeping the request inside the built-in CRUD, rendering, permission, validation, and response pipeline.

### When to Use
Use it near the beginning of a controller method to configure how Core handles the current request.

### Reference
`setDescription(array|string $params = [], ?string $fallback = null)`
...
### Behavior
`setDescription()` stores request-level configuration on the controller. Call it before the permission, rendering, or form-processing step that depends on it.

### Basic Usage
```php
$this->setDescription(phrase('Manage customer orders.'));
...
class Orders extends Core
{
    ...
    {
        $this->setTitle(phrase('Orders'))
            ->setDescription(phrase('Manage customer orders.'));
