`beforeUpdate()` runs custom logic before an update operation. It is used inside an Aksara controller as part of the Core method API.

### Purpose
`beforeUpdate()` runs custom logic before an update operation. It lets a controller customize Aksara Core behavior while keeping the request inside the built-in CRUD, rendering, permission, validation, and response pipeline.
...
### Behavior
`beforeUpdate()` is an override hook. Core calls it automatically at the matching point in the create, update, or delete flow when the method exists on the controller.

### Basic Usage
```php
protected function beforeUpdate()
{
    log_message('info', 'beforeUpdate hook executed.');
}
```
...
protected function beforeUpdate()
{
    if ($this->request->getPost('status') === 'paid' && ! get_userdata('is_admin')) {
        return throw_exception(403, phrase('Only administrators may mark an order as paid.'));
    }
}
```
