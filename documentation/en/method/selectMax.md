...
### Reference
`selectMax(string $column, ?string $alias = null)`
...
### Behavior
`selectMax()` records a query instruction on the controller. Core applies that instruction when `render()` compiles the final query. It does not execute a database query by itself.

### Basic Usage
```php
$this->selectMax('products.price', 'highest_price');
...
class Orders extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Orders'))
            ->selectMax('products.price', 'highest_price');
