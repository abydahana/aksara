`orGroupStart()` opens an OR grouped WHERE expression. It is used inside an Aksara controller as part of the Core method API.

### Purpose
`orGroupStart()` opens an OR grouped WHERE expression. It lets a controller customize Aksara Core behavior while keeping the request inside the built-in CRUD, rendering, permission, validation, and response pipeline.
...
### Behavior
`orGroupStart()` records a query instruction on the controller. Core applies that instruction when `render()` compiles the final query. It does not execute a database query by itself.

### Basic Usage
```php
$this->where('type', 'retail')->orGroupStart()->where('status', 'paid')->where('priority', 'high')->groupEnd();
...
class Orders extends BaseController
{
    public function index()
    {
        $this->setTitle(phrase('Orders'))
            ->where('type', 'retail')->orGroupStart()->where('status', 'paid')->where('priority', 'high')->groupEnd();
