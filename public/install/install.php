<?php
	...
	{
		exit(phrase('please_run') . ' "<code>composer install</code>" ' . phrase('from') . ' "<code>' . dirname(dirname(__DIR__)) . '</code>" ' . phrase('to_fetch_the_required_repository_before_we_start_the_installation_wizard') . '<br /><a href="//getcomposer.org"><b>' . phrase('click_here') . '</b></a> ' . phrase('to_download_the_composer_if_it_is_not_yet_installed_in_your_web_server'));
	}
	...
	elseif(file_exists(dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'config.php'))
	{
		if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
		{
			...
			(
				...
				(
					'status'						=> 301,
					'url'							=> (!$_SESSION['system']['mode'] && (!isset($_SESSION['unzip']) || !$_SESSION['unzip']) ? '../home/partial_error' : '../xhr/boot')
				)
			);
			exit;
		}
	...
	$html											= '
		...
		<div class="row">
			...
			<div class="col-sm-6">
				' . (file_exists(dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'config.php') ? '<a href="' . (!$_SESSION['system']['mode'] && !$_SESSION['unzip'] ? '../home/partial_error' : '../xhr/boot') . '" class="btn btn-warning btn-block font-weight-bold">' . phrase('launch_your_site') . '</a>' : '<a href="install.php?validate_config=1" class="btn btn-warning btn-block font-weight-bold --xhr">' . phrase('revalidate_configuration') . '</a>') . '
			</div>
		</div>
	';
