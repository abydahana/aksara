...
else if(!is_writable(dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'writable') || !is_writable(dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'writable' . DIRECTORY_SEPARATOR . 'session') || !is_writable(dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'uploads'))
{
	exit('<center>You need to make the following directory and it\'s subdirectories to be writable recursively: ' . (!is_writable(dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'writable') || !is_writable(dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'writable' . DIRECTORY_SEPARATOR . 'session') ? '<br /><code style="color:red">' . dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'writable' . '</code>' : null) . (!is_writable(dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'uploads') ? '<br /><code style="color:red">' . dirname(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'uploads' . '</code>' : null) . '<br />Please <a href="//github.com/abydahana/Aksara/issues/2" target="_blank"><b>click here</b></a> to get support related to this error.</center>');
}
