...
function go_to(path, params) {
  ...
  return url.replace(/\/$/, '') + (path ? '/' + path : '') + (query_params ? '?' + $.param(query_params) : '');
}
...
function throw_exception(code, message, target, redirect) {
  ...
  Twig.extendFunction('current_page', function(path, params, unset) {
    ...
    let queryString = Object.keys(query_params).length ? '?' + $.param(query_params) : '';
...
function reactivate(individual, ignore_self) {
    ...
    $(window).on('resize', function() {
      ...
      .on('scroll', function(e) {
        if ($('.navbar').first().next('[data-role=content]').children('.carousel').length || $('.navbar').first().next('[data-role=content]').children('.leading').length) {
          ...
          let element = ($('.navbar').first().next('[data-role=content]').children('.carousel').length ? $('.navbar').first().next('[data-role=content]').children('.carousel') : ($('.navbar').first().next('[data-role=content]').children('.leading').length ? $('.navbar').first().next('[data-role=content]').children('.leading') : null));
   ...
   * Select2 plugin
   */
  if ($('select[data-role=select]').length && (!individual.length || individual.includes('select')) && !ignore_self) {
    require.css([
    ...
    require.js([
      ...
      $('select[data-role=select]').each(function() {
        ...
        const extra = group.find('.input-group-text').toArray().reduce(
            (sum, el) => sum + $(el).outerWidth(true),
            0
        );
        const config = {
          theme: 'bootstrap-5',
        ...
        $(this).select2(config).on('change', function(e) {
	      ...
	      $('body').off('change.ajaxSelectDepends').on('change.ajaxSelectDepends', 'select[data-role=select]', function() {
	        ...
	        form.find('select[data-role=select][data-depends]').each(function() {
	          const depends = ($(this).data('depends') || '').toString().split(',').map((field) => field.trim());
   ...
   */
  if ($('[data-role=wysiwyg]').length && (!individual.length || individual.includes('wysiwyg')) && !ignore_self) {
    require.css([
   ...
   */
  if ($('[data-role=videoplayer]').length && (!individual.length || individual.includes('videoplayer')) && !ignore_self) {
    require.css([
   ...
   */
  if ($('[data-role=calendar]').length && (!individual.length || individual.includes('calendar')) && !ignore_self) {
    require.css([
   ...
   */
  if ($('[data-role=money]').length && (!individual.length || individual.includes('money')) && !ignore_self) {
    require.js([
      ...
      let decimalCommaLanguages = [
        'id', 'de', 'es', 'it', 'nl', 'pt', 'tr', 'vi', 'da', 'el', 'is', 'ro', 'sl'
      ];
   ...
   * Uploader plugin
   */
  if ($('[data-role=uploader]').length && (!individual.length || individual.includes('uploader')) && !ignore_self) {
        ...
        if (this.files && this.files[0]) {
          ...
          reader.onload = function(response) {
            if (!response.target.error) {
              ...
              context.clone().css('display', 'none').removeAttr('data-role class id').appendTo(context.closest('.uploader-input').prev('.uploader-list'));
   ...
   * Checkbox
   */
  if ($('input[data-role=checker]').length && (!individual.length || individual.includes('checker')) && !ignore_self) {
   ...
   * Autocomplete plugin
   */
  if ($('[data-role=autocomplete]').length && (!individual.length || individual.includes('autocomplete')) && !ignore_self) {
    require.js([
   ...
   */
  if ($('[data-role=map]').length && (!individual.length || individual.includes('map')) && !ignore_self) {
    $('[data-role=map]').each(function() {
      ...
      require.js([
        ...
        config.base_url + 'assets/openlayers/ol-dropfile/ol-dropfile.min.js',
   ...
   */
  if ($('[data-role=iconpicker]').length && (!individual.length || individual.includes('iconpicker')) && !ignore_self) {
    require.css([
      config.base_url + 'assets/bootstrap-iconpicker/css/bootstrap-iconpicker.min.css'
    ]);
    require.js([
      config.base_url + 'assets/bootstrap-iconpicker/js/iconset/materialdesignicons.3.9.97.min.js',
   ...
   */
  if ($('[data-role=sortable-menu]').length && (!individual.length || individual.includes('sortable-menu')) && !ignore_self) {
   ...
   * Sortable
   */
  if ($('[data-role=sortable]').length && (!individual.length || individual.includes('sortable')) && !ignore_self) {
   ...
   * Widgets
   */
  if ($('[data-role=widget]').length && (!individual.length || individual.includes('widget')) && !ignore_self) {
