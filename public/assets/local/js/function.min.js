...
function _reactivate(is_index)
{
	...
	if($('.full-height').length)
	{
		$('.full-height').css
		({
			minHeight: $(window).outerHeight(true) - (($('.navbar:not(.bg-transparent)').length ? $('.navbar:not(.bg-transparent)').outerHeight(true) : 0) + ($('#breadcrumb-wrapper').length ? $('#breadcrumb-wrapper').outerHeight(true) : 0) + ($('.alias-table-header').length ? $('.alias-table-header').outerHeight(true) : 0) + ($('.alias-table-toolbar').length ? $('.alias-table-toolbar').outerHeight(true) : 0) + ($('.alias-description-collapse').length ? $('.alias-description-collapse').outerHeight(true) : 0) + ($('.opt-btn').length ? $('.opt-btn').outerHeight(true) : 0))
		})
	}
	...
	if(UA != 'mobile')
	{
		...
		({
			...
			axis: 'y',
			scrollInertia: 200,
			setHeight: $(window).outerHeight(true) - (($('.navbar').length ? $('.navbar').outerHeight(true) : 0) + ($('#breadcrumb-wrapper').length ? $('#breadcrumb-wrapper').outerHeight(true) : 0) + ($('.alias-announcements-ticker').length ? 30 : 0)),
	 ...
	 */
	if($('.navbar:visible').first().next('#content-wrapper').children('.carousel').length || $('.navbar:visible').first().next('#content-wrapper').children('.leading').length)
	{
	 ...
	 */
	if(typeof _retrigger !== 'undefined')
	{
		for (var i = 0; i < _retrigger.length; i++)
		{
			if(typeof _retrigger[i] === 'function')
			{
				_retrigger[i].call()
			}
			else
