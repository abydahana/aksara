...
function reactivate(individual, ignore_self) {
    ...
    require.js([
      ...
      $('select[role=select]').each(function() {
        ...
        $(this).select2(config).on('change', function(e) {
	      ...
	      $('body').off('change.ajaxSelectDepends').on('change.ajaxSelectDepends', 'select[role=select]', function() {
	        ...
	        form.find('select[role=select][data-depends]').each(function() {
  ...
  if ($('[role=money]').length && (!individual.length || individual.includes('money')) && !ignore_self) {
    require.js([
      ...
      $('[role=money]').autoNumeric('init', moneyOptions);
              ...
              <div class="row mb-2 align-items-center">
                <div class="col-2 pe-0">
                  <a href="${val.url}" target="_blank" download="${val.name}">
                    ${(val.icon ? '<img src="' + val.icon + '" class="img-fluid rounded" style="max-height:32px" alt="' + (val.name || '') + '" />' : '<button type="button" class="btn btn-primary pe-2 ps-2">' + extension + '</button>')}
                  </a>
                </div>
                  ...
                  <div class="row mb-2 align-items-center">
                    <div class="col-2 pe-0">
                      <a href="${response.target.result}" target="_blank" download="${file.name}">
                        ${($.inArray(extension, ['.jpg', '.jpeg', '.png', '.gif']) !== -1 ? '<img src="' + response.target.result + '" class="img-fluid rounded" style="max-height:32px" alt="' + (file.name || '') + '" />' : '<button type="button" class="btn btn-primary pe-2 ps-2">' + extension + '</button>')}
                      </a>
                    </div>
...
function _viewport_modifier() {
   ...
   */
  fullHeight = window.innerHeight - (($('[role=header]:visible').outerHeight(true) ?? 0) + ($('[role=footer]:visible').outerHeight(true) ?? 0) + ($('[role=breadcrumb]:visible').outerHeight(true) ?? 0));

  $('.full-height').css({
    minHeight: fullHeight - (($('[role=meta]:visible').outerHeight(true) ?? 0) + ($('[role=toolbar]:visible').outerHeight(true) ?? 0) + ($('[role=pagination]:visible').outerHeight(true) ?? 0))
  });
  ...
  if (UA != 'mobile') {
    ...
    require.js([
      ...
      $('[role=sidebar], [role=table], [role=grid], .pretty-scrollbar').each(function() {
