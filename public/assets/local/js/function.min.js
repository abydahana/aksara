...
function reactivate(individual, ignore_self) {
    ...
    .on('scroll', function(e) {
        if ($('.navbar').first().next('[role=content]').children('.carousel').length || $('.navbar').first().next('[role=content]').children('.leading').length) {
            ...
            var element = ($('.navbar').first().next('[role=content]').children('.carousel').length ? $('.navbar').first().next('[role=content]').children('.carousel') : ($('.navbar').first().next('[role=content]').children('.leading').length ? $('.navbar').first().next('[role=content]').children('.leading') : null)),
    ...
    if ($('select[role=select]').length && (! individual.length || individual.includes('select')) && ! ignore_self) {
        ...
        require.js([
            ...
            $('select[role=select]').each(function() {
                if ($(this).next('.select2').length || $(this).hasClass('no-js') || ! $(this).data('relation')) {
                    return;
                }
                ...
                .on('change', function(e) {
                    ...
                    var autosubmit = $(this).closest('form').attr('data-autosubmit');
    ...
    if ($('[role=calendar]').length && (! individual.length || individual.includes('calendar')) && ! ignore_self) {
        ...
        require.js([
            ...
            $('[role=calendar]').each(function() {
                let context = $(this);
                var calendar = new FullCalendar.Calendar(document.getElementById(context.attr('id')), {
                    timeZone: 'GMT',
     ...
     */
    if ($('[role=price]').length && (! individual.length || individual.includes('price')) && ! ignore_self) {
        require.js([
            config.base_url + 'assets/price/price.min.js'
        ], function() {
            $('[role=price]').autoNumeric('init', {
                vMin: '-999999999.99'
            });
    ...
    if ($('[role=map]').length && (! individual.length || individual.includes('map')) && ! ignore_self) {
        $('[role=map]').each(function() {
            ...
            if (config.openlayers_search_provider) {
                require.js('https://maps.googleapis.com/maps/api/js?key=' + config.openlayers_search_key + '&libraries=places&language=' + config.language + '&callback=initAutocomplete')
            }
        ...
        $('[role=sortable-menu]').each(function() {
            ...
            require.js([
                ...
                .on('click', '.item-add', function(e) {
                    ...
                    var new_item = context.find('li.item-placeholder').prop('outerHTML'),
...
function initAutocomplete() {
    if (! document.getElementById('autocomplete') || typeof google === 'undefined') return;

    let autocomplete = new google.maps.places.Autocomplete(
        document.getElementById('autocomplete'), {
            componentRestrictions: {
                country: config.language ?? 'id'
            },
        ...
        geocoder.geocode({
            ...
            function(results, status) {
                if (status === 'OK') {
                    ...
                    if (document.getElementById('autocomplete').getAttribute('data-finder-url') && typeof openlayers !== 'undefined') {
                        xhr = $.ajax({
                            url: document.getElementById('autocomplete').getAttribute('data-finder-url'),
                            method: 'POST',
                            data: {
