...
const parser = (function() {
    return {
        ...
        render: function(context, response, popstate) {
            if (typeof response !== 'object') {
                /**
                 * Response is not valid object, use error template
                 */
                $('.modal.show').modal('hide');
                $(config.wrapper.content).html(this.render(context, 'core/error.twig', {}));
            ...
            if (typeof response.message !== 'undefined') {
                ...
                if (typeof response.meta.title === 'undefined' || ! response.meta.title) {
                    ...
                    $('[role=icon]').removeAttr('class').addClass('mdi mdi-alert-outline');
                    $('.modal#dynamic-modal-' + context.data('identifier')).find('.modal-content').addClass('rounded-4');
                    $('.modal#dynamic-modal-' + context.data('identifier')).find('.modal-header').remove();
                }

                if (context.data('identifier') && '.modal#dynamic-modal-' + context.data('identifier').length) {
                    // Apply content to existing modal
                    $('.modal#dynamic-modal-' + context.data('identifier')).find('.modal-header').find('.modal-title-text').text(response.meta.icon);
                ...
                if (typeof response.breadcrumb !== 'undefined') {
                    ...
                    $.each(response.breadcrumb, function(key, val) {
                        $('<li class="breadcrumb-item"><a href="' + val.url + '" class="--xhr">' + (val.icon ? '<i class="' + val.icon + '"></i>' : '') + val.label + '</a></li>').appendTo('[role=breadcrumb] ol')
                    })
                }
