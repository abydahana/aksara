...
var UA												= ($(window).outerWidth() < 1024 ? 'mobile' : 'desktop'),
	...
	{
		if(typeof response.results.grid !== 'undefined')
		{
	...
	grid: function(response)
	{
		...
		if(typeof response.total !== 'undefined' && response.total && typeof response.results.table_data !== 'undefined')
		{
			...
			$.each(response.results.table_data, function(key, val)
			{
				...
				$.each(val, function(field, params)
				{
					...
					{
						var images					= JSON.parse(params.original);
						...
						if(images)
						{
							...
							$.each(images, function(src, alt)
							{
								...
								slideshow			+= '<div class="carousel-item rounded' + (!num ? ' active' : '') + '"><a href="' + config.base_url + 'uploads/' + response.results.grid.path + '/thumbs/' + src + '" target="_blank"><div class="clip gradient-top rounded-top"></div><img src="' + config.base_url + 'uploads/' + response.results.grid.path + '/thumbs/' + src + '" class="d-block rounded w-100" alt="' + alt + '"></a><div class="carousel-caption"><p>' + alt + '</p></div></div>';
		...
		else
		{
			 ...
			 * Uh oh! No data were found, throw the notification into table row
			 */
			grid									= '<div class="col-lg-12"><div class="text-center full-height d-flex align-items-center justify-content-center"><div><i class="mdi mdi-dropbox mdi-5x text-muted"></i><br /><p class="lead text-muted">' + (phrase.no_matching_record_were_found ? phrase.no_matching_record_were_found : 'No matching record were found') + '</p></div></div></div>';
		}
						...
						'<form action="' + response.url + '" method="POST" class="--xhr-form ml-2 d-none d-sm-none d-md-block d-lg-block d-xl-block">' +
							'<div class="input-group">' +
								'<select name="limit" class="form-control form-control-sm">' +
									'<option value="25"' + (!response.per_page ? ' selected' : '') + '>25</option>' +
									'<option value="50"' + (50 == response.per_page ? ' selected' : '') + '>50</option>' +
									'<option value="100"' + (100 == response.per_page ? ' selected' : '') + '>100</option>' +
									'<option value="200"' + (200 == response.per_page ? ' selected' : '') + '>200</option>' +
									'<option value="500"' + (500 == response.per_page ? ' selected' : '') + '>500</option>' +
								'</select>' +
	...
	form: function(_this, response, modal_id)
	{
		...
		{
			component.templates(response, response.method, modal_id);
									...
									'<div class="card-body">' +
										'<form action="' + _this.href + '" method="POST" class="--validate-form" enctype="multipart/form-data">' +
											...
											'<p class="pb-3 text-center">' +
												(phrase.please_enter_your_account_information_to_sign_in ? phrase.please_enter_your_account_information_to_sign_in : 'Use your account information to start session') +
											'</p>' +
										...
										'<div>' +
											...
											'<p class="lead text-center">' +
												'<a href="' + config.base_url + 'auth/google" class="btn btn-outline-danger btn-block">' +
													...
													(phrase.sign_in_with_google ? phrase.sign_in_with_google : 'Sign With Google') +
												'</a>' +
											'</p>'
											: '') +
											...
											'<p class="lead text-center">' +
												'<a href="' + config.base_url + 'auth/facebook" class="btn btn-outline-primary btn-block">' +
													...
													(phrase.sign_in_with_facebook ? phrase.sign_in_with_facebook : 'Sign With Facebook') +
												'</a>' +
											'</p>'
											: '') +
											'<p class="lead text-center">' +
	...
	templates: function(response, method, modal_id)
	{
		if($.inArray(method, ['read']) !== -1)
		{
			...
			else
			{
				...
				$(modal_id).find('.modal-body').html((map ? map : '<br />') + output),
		...
		else if($.inArray(method, ['create', 'update']) !== -1)
		{
			...
			if(sessionStorage.getItem('form'))
			{
				...
				(output ? $(modal_id).find('.modal-body').html(output) : null),
