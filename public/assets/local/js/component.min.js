...
var UA												= ($(window).outerWidth() < 1024 ? 'mobile' : 'desktop'),
	...
	table: function(response)
	{
		...
		if(typeof response.total !== 'undefined' && response.total > 0 && typeof response.results.table_data !== 'undefined')
		{
			/**
		...
		if(response)
		{
			...
			$.each(response.results.form_data, function(field, params)
			{
				...
				if(typeof response.results.merged_field[field] !== 'undefined')
				{
					...
					{
						if(typeof response.results.set_heading[field] !== 'undefined')
						{
	...
	modal: function(_this)
	{
		...
		.done(function(response)
		{
			...
			if(response)
			{
				...
				(response && response.meta && response.meta.modal_size ? $('.modal-dialog').addClass(response.meta.modal_size) : null),
			...
			$('#dynamic-modal-' + identifier).find('.modal-title').find('.mdi').removeClass('mdi mdi-loading mdi-spin').addClass((response && response.meta ? response.meta.icon : 'mdi mdi-alert-outline')),
			$('#dynamic-modal-' + identifier).find('.modal-title').find('.modal-title-text').text((response && response.meta ? response.meta.title : (phrase.title_was_not_set ? phrase.title_was_not_set : 'Title was not set')))
		})
	},
												...
												'<div class="row mt-3">' +
													'<div class="col-7">' +
														'<label>' +
															...
															(phrase.remember_session ? phrase.remember_session : 'Remember session') +
														'</label>' +
													'</div>' +
													'<div class="col-5">' +
