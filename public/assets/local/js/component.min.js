...
var UA												= ($(window).outerWidth() < 1024 ? 'mobile' : 'desktop'),
	...
	form: function(_this, response, modal_id)
	{
		...
		if(response)
		{
			...
			{
				if($.inArray('hidden', params.type) !== -1)
				{
		...
		else if($.inArray(method, ['create', 'update']) !== -1)
		{
			...
			if(sessionStorage.getItem('form'))
			{
				...
				if(!rendered)
				{
								...
								'<div class="' + (typeof response.results.field_size[val] !== 'undefined' ? response.results.field_size[val] : 'col') + '">' +
									'<div class="form-group">' +
										'<label class="text-muted d-block mb-0" for="' + val + '_input">' +
											response.results.form_data[val]['label'] +
											(response.results.form_data[val]['tooltip'] ? ' <i class="mdi mdi-information-outline text-primary" data-toggle="tooltip" title="' + response.results.form_data[val]['tooltip'] + '"></i>' : '') +
											(params.required ? '<b class="text-danger"> *</b>' : '') +
										'</label>' +
										(response.results.form_data[val]['prepend'] || response.results.form_data[val]['append'] ? '<div class="input-group">' : '') + (response.results.form_data[val]['prepend'] ? '<div class="input-group-prepend"><span class="input-group-text">' + response.results.form_data[val]['prepend'] + '</span></div>' : '') + response.results.form_data[val]['content'] + (response.results.form_data[val]['append'] ? '<div class="input-group-append"><span class="input-group-text">' + response.results.form_data[val]['append'] + '</span></div>' : '') + (response.results.form_data[val]['prepend'] || response.results.form_data[val]['append'] ? '</div>' : '') +
									'</div>' +
								'</div>';
					...
					output							= (map ? map : '<br />') + output + hidden + '<div class="--validation-callback mb-0"></div>';
				}
