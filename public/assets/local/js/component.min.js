			...
			$.each(response.results.table_data, function(key, val)
			{
				...
				$.each(val, function(field, params)
				{
					...
					if(typeof response.results.grid.thumbnail !== 'undefined' && field == response.results.grid.thumbnail && params.type.filter(function(n){return ['image', 'images'].indexOf(n) !== -1}))
					{
						...
						{
							slideshow				= '<div class="carousel-item rounded active"><a href="' + config.base_url + 'uploads/' + response.results.grid.path + '/thumbs/' + images + '" target="_blank"><div class="clip gradient-top rounded-top"></div><img src="' + config.base_url + 'uploads/' + response.results.grid.path + '/thumbs/' + images + '" class="d-block rounded w-100" alt="..."></a><div class="carousel-caption"></div></div>';
						}
					}
					else
					{
