	...
	form: function(_this, response, modal_id)
	{
		...
		({
			...
			statusCode:
			{
				...
				{
					$('#form-modal').find('.modal-body').addClass('bg-dark text-center p-3 rounded-bottom').html('<p><code>' + response + '</code></p><p class="mt-5"><a href="#" class="btn btn-outline-light btn-sm --report-to-admin">' + (phrase.report_to_admin ? phrase.report_to_admin : 'Report to Admin') + '</a></p>');
		...
		({
			...
			statusCode:
			{
				...
				500: function(response, status, error)
				{
					...
					$('#read-modal').find('.modal-body').addClass('bg-dark text-center p-3 rounded-bottom').html('<p><code>' + response + '</code></p><p class="mt-5"><a href="#" class="btn btn-outline-light btn-sm --report-to-admin">' + (phrase.report_to_admin ? phrase.report_to_admin : 'Report to Admin') + '</a></p>');
			...
			else
			{
				...
				$.each(response.results.form_data, function(field, params)
				{
							...
							'<div class="' + (typeof response.results.field_size[val] !== 'undefined' ? response.results.field_size[val] : 'col') + '">' +
								'<div class="form-group">' +
									'<label class="text-muted d-block mb-0" for="' + val + (is_form ? '_input' : '_label') + '">' +
										response.results.form_data[val]['label'] +
										(is_form && response.results.form_data[val]['tooltip'] ? ' <i class="mdi mdi-information-outline text-primary" data-toggle="tooltip" title="' + response.results.form_data[val]['tooltip'] + '"></i>' : '') +
										(is_form && params.required ? '<b class="text-danger"> *</b>' : '') +
									'</label>' +
									(is_form ? (response.results.form_data[val]['prepend'] || response.results.form_data[val]['append'] ? '<div class="input-group">' : '') + (response.results.form_data[val]['prepend'] ? '<div class="input-group-prepend"><span class="input-group-text">' + response.results.form_data[val]['prepend'] + '</span></div>' : '') + response.results.form_data[val]['content'] + (response.results.form_data[val]['append'] ? '<div class="input-group-append"><span class="input-group-text">' + response.results.form_data[val]['append'] + '</span></div>' : '') + (response.results.form_data[val]['prepend'] || response.results.form_data[val]['append'] ? '</div>' : '') : '<p id="' + val + '_label" class="text-break-word">' + (phrase.loading ? phrase.loading : 'Loading...') + '</p>') +
								'</div>' +
							'</div>';
					...
					else
					{
						...
						output						= '<div class="form-group"><label class="text-muted d-block mb-0" for="' + field + (is_form ? '_input' : '_label') + '">' + params.label + (is_form && params.tooltip ? ' <i class="mdi mdi-information-outline text-primary" data-toggle="tooltip" title="' + params.tooltip + '"></i>' : '') + (params.required ? '<b class="text-danger"> *</b>' : '') + '</label>' + (is_form ? (params.prepend || params.append ? '<div class="input-group">' : '') + (params.prepend ? '<div class="input-group-prepend"><span class="input-group-text">' + params.prepend + '</span></div>' : '') + params.content + (params.append ? '<div class="input-group-append"><span class="input-group-text">' + params.append + '</span></div>' : '') + (params.prepend || params.append ? '</div>' : '') : params.content) + '</div>';
