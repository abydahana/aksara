				...
				'<div class="col-sm-6">' +
					'<nav class="d-flex justify-content-center justify-content-sm-end justify-content-md-end justify-content-lg-end justify-content-xl-end" aria-label="Page navigation">' +
						'<ul class="pagination pagination-sm">' +
							output +
						'</ul>' +
						'<form action="' + go_to('', {'per_page': ''}) + '" method="POST" class="--xhr-form ml-2 d-none d-sm-none d-md-block d-lg-block d-xl-block">' +
							'<div class="input-group">' +
								...
								'</select>' +
								'<input type="number" name="per_page" class="form-control form-control-sm text-center" value="' + (get_query_string('per_page') ? get_query_string('per_page') : 1) + '" min="1" max="' + response.last_page + '"' + (response.last_page <= response.per_page ? ' disabled' : '') + ' />' +
								'<div class="input-group-append">' +
									'<button type="submit" class="btn btn-sm btn-primary' + (response.last_page <= response.per_page ? ' disabled' : '') + '">' +
	...
	view: function(_this, response)
	{
		...
		.done(function(response)
		{
			...
			else
			{
				...
				$('#read-modal').find('.modal-body').html((map ? map : '<br />') + output),
