					...
					if(typeof response.results.grid.thumbnail !== 'undefined' && field == response.results.grid.thumbnail && params.type.filter(function(n){return ['image', 'images'].indexOf(n) !== -1}))
					{
						...
						if(typeof images === 'object')
						{
							...
							$.each(images, function(src, alt)
							{
								...
								{
									slideshow		+= '<div class="carousel-item rounded' + (!num ? ' active' : '') + '"><a href="' + (typeof response.results.grid.url !== 'undefined' ? response.results.grid.url : config.base_url + 'uploads/' + response.results.grid.path + '/thumbs/' + src) + '"' + (typeof response.results.grid.url !== 'undefined' && !response.results.grid.new_tab ? ' class="--xhr"' : ' target="_blank"') + '><div class="clip gradient-top rounded-top"></div><img src="' + config.base_url + 'uploads/' + response.results.grid.path + '/thumbs/' + src + '" class="d-block rounded w-100" alt="' + alt + '"></a><div class="carousel-caption"><p>' + alt + '</p></div></div>';
						...
						{
							slideshow				= '<div class="carousel-item rounded active"><a href="' + (typeof response.results.grid.url !== 'undefined' ? response.results.grid.url : config.base_url + 'uploads/' + response.results.grid.path + '/thumbs/' + images) + '"' + (typeof response.results.grid.url !== 'undefined' && !response.results.grid.new_tab ? ' class="--xhr"' : ' target="_blank"') + '><div class="clip gradient-top rounded-top"></div><img src="' + config.base_url + 'uploads/' + response.results.grid.path + '/thumbs/' + images + '" class="d-block rounded w-100" alt="..."></a><div class="carousel-caption"></div></div>';
						}
					}
					else
					{
						...
						'<form action="' + response.url + '" method="POST" class="--xhr-form ml-2 d-none d-sm-none d-md-block d-lg-block d-xl-block">' +
							'<div class="input-group">' +
								'<select name="limit" class="form-control form-control-sm">' +
									'<option value="' + response.per_page + '"' + (!response.per_page ? ' selected' : '') + '>' + response.per_page + '</option>' +
									'<option value="' + (response.per_page * 2) + '"' + ((response.per_page * 2) == response.per_page ? ' selected' : '') + '>' + (response.per_page * 2) + '</option>' +
									'<option value="' + (response.per_page * 4) + '"' + ((response.per_page * 4) == response.per_page ? ' selected' : '') + '>' + (response.per_page * 4) + '</option>' +
									'<option value="' + (response.per_page * 8) + '"' + ((response.per_page * 8) == response.per_page ? ' selected' : '') + '>' + (response.per_page * 8) + '</option>' +
									'<option value="' + (response.per_page * 20) + '"' + ((response.per_page * 20) == response.per_page ? ' selected' : '') + '>' + (response.per_page * 20) + '</option>' +
								'</select>' +
