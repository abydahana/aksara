...
var UA												= ($(window).outerWidth() < 1024 ? 'mobile' : 'desktop'),
	...
	table: function(response)
	{
		...
		if(typeof response.results.columns !== 'undefined')
		{
			$.each(response.results.columns, function(key, val)
			{
				...
				fields								+= '<th class="border-top-0' + ('right' == val['align'] ? ' text-end' : '') + '"><a href="' + go_to('', parameter) + '" class="--xhr' + (key == get_query_string('order') ? ' text-primary' : ' text-default') + '"><i class="mdi mdi-sort-' + (key == get_query_string('order') && 'asc' == get_query_string('sort') ? 'ascending' : 'descending') + ' float-end pt-1' + (key == get_query_string('order') ? ' text-primary' : ' text-muted') + '"></i> ' + val['label'] + '</a></th>';
				colspan++;
			})
		}
					...
					'<div class="modal-content border">' +
						...
						'<div class="modal-footer" style="display:block">' +
							'<div class="row">' +
								...
								'<div class="col-6">' +
									'<div class="d-grid">' +
										'<a href="' + _this.attr('href') + '" class="btn btn-danger --delete-anyway"' + (_this.attr('data-bulk-delete') ? ' data-bulk-delete="true"' : '') + '>' +
											'<i class="mdi mdi-check"></i> ' +
	...
	templates: function(response, method, modal_id)
	{
		...
		if($.inArray(method, ['read']) !== -1)
		{
			...
			(typeof response.meta !== 'undefined' && typeof response.meta.description !== 'undefined' ? $(modal_id).find('.modal-body').addClass('pt-0').prepend('<div class="alert alert-info mb-0">' + response.meta.description + '</div>') : null),
