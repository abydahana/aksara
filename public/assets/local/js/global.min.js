...
$(document).ready(function() {
  ...
  $('body:not(.no-ajax)').on('submit', 'form[method=GET]:not(.no-ajax)', function(e) {
      ...
      // 1. Create a URL object from the form action (or current location).
      // passing 'window.location.origin' as the second argument ensures relative paths work.
      // This automatically loads existing query strings (e.g., ?sort=date).
      const urlObj = new URL(form.attr('action') || window.location.href, window.location.origin);

      const formData = new FormData(form[0]);
      const processedKeys = new Set();

      // 2. Iterate over form inputs
      for (let [key, value] of formData.entries()) {
          // LOGIC: "If input matches, replace existing."

          // We track keys in a Set to handle arrays (like checkboxes name="ids[]").
          // We only delete the old parameter from the URL *once* when we first encounter the key.
          if (!processedKeys.has(key)) {
              urlObj.searchParams.delete(key); // Remove the old value from URL
              processedKeys.add(key);
          }

          // Add the new value from the form
          urlObj.searchParams.append(key, value);
      }

      // 3. Handle Submit Button (if it has name/value)
      const submitButton = form.find('button[type=submit]:focus, input[type=submit]:focus');
      if (submitButton.length) {
          const btnName = submitButton.attr('name');
          const btnValue = submitButton.attr('value');
          if (btnName && btnValue) {
              // Same logic: overwrite if exists
              if (!processedKeys.has(btnName)) {
                  urlObj.searchParams.delete(btnName);
              }
              urlObj.searchParams.append(btnName, btnValue);
          }
      }

      // 4. Generate Final URL and Trigger
      // urlObj.toString() automatically handles the '?' and '&' structure.
      $('<a href="' + urlObj.toString() + '" class="--xhr"></a>')
          .appendTo('body')
          .trigger('click')
          .remove();
  });
