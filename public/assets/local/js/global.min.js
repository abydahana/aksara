...
$(document).ready(function()
{
			...
			({
				...
				complete: function(progress)
				{
					if(!$(this).hasClass('--prevent-modify'))
					{
						...
						$(this).parents('ul').parents('.nav-item').addClass('is-parent'),
	...
	$('body').on('submit', '.--validate-form', function(e)
	{
		...
		$('[role=datetimepicker]').each(function()
		{
			...
			{
				var date							= new Date($(this).data('DateTimePicker').viewDate().toLocaleString());
				
				formData.set($(this).attr('name'), date.toISOString().split('T')[0] + ' ' + date.toTimeString().split(' ')[0])
			}
		}),
		...
		({
			...
			statusCode:
			{
				...
				400: function(response, status, error)
				{
					...
					$(this).find('.--validation-callback').addClass('alert alert-warning mt-4 mb-3'),
	...
	$('body').on('submit', '.--xhr-form', function(e)
	{
		...
		$('input[role=datepicker]', $(this)).each(function()
		{
			...
			formData.append($(this).attr('name'), date);
		}),
		...
		({
			...
			statusCode:
			{
				301: function(response, status, error)
				{
					...
					/* refresh table data */
					($('.--btn-refresh').length ? $('.--btn-refresh').trigger('click') : (typeof response.target !== 'undefined' ? $('<a href="' + response.target + '" class="--xhr"></a>').appendTo('body').trigger('click').remove() : null)),
				...
				403: function(response, status, error)
				{
					...
					(typeof response.target !== 'undefined' ? $('<a href="' + response.target + '" class="--xhr"></a>').appendTo('body').trigger('click').remove() : null),
