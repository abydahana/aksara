...
$(document).ready(function() {
     ...
     */
    $('body:not(.no-ajax)').on('submit', 'form[method=POST]:not(.no-ajax)', function(e) {
    ...
    $('body:not(.no-ajax)').on('click touch', 'a.--xhr, a.--modal', function(e) {
        ...
        .done(function(response) {
            ...
            if (typeof response.toggle_html !== 'undefined') {
    ...
    $('body').on('change', 'select[data-to-change]', function(e) {
        ...
        .done(function(response) {
            if (typeof response.suggestions !== 'undefined') {
                ...
                $(this).closest('form').find('select[name=' + response.selector + ']').html(response.content).attr('disabled', false);
            }
        })
        .fail(function(response, textStatus, errorThrown) {
            if (textStatus === 'abort') {
            ...
            .done(function(response) {
                if (typeof response === 'object') {
                    ...
                    $.each(response, function(key, val) {
                        $('<li class="nav-item"><a href="' + config.base_url + 'xhr/language/' + val.code + '" class="nav-link"><i class="mdi mdi-translate"></i> <span>' + val.language + '</span></a></li>').appendTo(context.closest('li').find('ul'));
                    });
                }
            })
            .fail(function(response, textStatus, errorThrown) {
