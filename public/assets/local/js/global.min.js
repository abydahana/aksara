...
var UA												= ($(window).outerWidth() < 1024 ? 'mobile' : 'desktop'),
	...
	alert											= new Audio(config.asset_url + 'local/sound/alert.mp3');
...
$(document).ready(function()
{
	...
	$(window).bind("popstate", function(e)
	{
		...
		{
			/* throw log to console */
			$(config.content_wrapper).html('<div class="container-fluid"><p class="mt-3 mb-3 p-3 bg-dark rounded"><code>' + (error ? error : (phrase.consider_to_check_your_internet_connection ? phrase.consider_to_check_your_internet_connection : 'Consider to check your internet connection')) + '</code></p></div>'),
								...
								'<div class="modal-header">' +
									'<h5 class="modal-title" id="alert-modal-title">' +
										...
										'<span class="modal-title-text">' +
											(response.exception.title ? response.exception.title : (phrase.loading ? phrase.loading : 'Loading...')) +
										'</span>' +
									'</h5>' +
			...
			if(response.url)
			{
				...
				.done(function(_response)
				{
					if(typeof _response.results !== 'undefined' && _response.results.form_data !== 'undefined')
					{
						...
						{
							$('<div class="col-sm-6"><label class="d-block"><input type="checkbox" name="parameter[' + key + ']"  value="1"' + (val.required ? ' checked readonly' : (typeof response.parameter !== 'undefined' && typeof response.parameter[key] !== 'undefined' && typeof response.parameter[key] ? ' checked' : '')) + '/> ' + val.label + (val.required ? ' <span class="text-danger text-sm">(' + (phrase.required ? phrase.required : 'Required') + ')</span>' : '') + '</label></div>').appendTo('.data-appended')
						})
					}
					else
					{
