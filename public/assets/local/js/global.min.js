...
$(document).ready(function()
{
	...
	$('body').on('submit', '.--validate-form', function(e)
	{
		...
		.done(function(response)
		{
			...
			{
				$(response.html).prependTo((typeof response.in_context !== 'undefined' && response.in_context ? $(this).closest(response.append_to) : response.append_to))
			}
	...
	$('body').on('change', 'select.get-dropdown-content', function(e)
	{
		...
		.done(function(response)
		{
			...
			{
				$(response.html).addClass('dropdown-content mt-3').prependTo($(this).closest('.form-group'))
			}
		})
	}),
		...
		if(!$(this).closest('li').find('ul').find('li').length)
		{
			...
			.done(function(response)
			{
				if(typeof response === 'object')
				{
					...
					{
						$('<li class="nav-item"><a href="' + config.base_url + '/xhr/language/' + val.code + '" class="nav-link --xhr"><i class="mdi mdi-translate"></i> ' + val.language + '</a></li>').appendTo(_this.closest('li').find('ul'))
					})
				}
			})
		}
