...
$(document).ready(function() {
  ...
  /**
   * Toggle theme dark/light (Bootstrap 5)
   */
  ...
  $('body').on('click touch', '[data-toggle=theme]', function(e) {
    e.preventDefault();
  ...
  $('body:not(.no-ajax)').on('click touch', 'a.--xhr, a.--modal', function(e) {
    ...
    if (!$(this).attr('href') || $(this).attr('href') === '#' || $(this).attr('href') === 'javascript:void(0)' || $(this).attr('data-bs-toggle') === 'dropdown' || $(this).hasClass('dropdown-toggle') || $(this).attr('href').indexOf(config.base_url) === -1 || $(this).attr('target')) {
      return;
    }
  ...
  $('body').on('click touch', '[data-role=expand]', function(e) {
    ...
    $('[data-role=table], [role=form]').height(function(index, height) {
      return $(window).outerHeight(true) - (($('[data-role=header]:visible').outerHeight(true) ?? 0) + ($('[data-role=breadcrumb]:visible').outerHeight(true) ?? 0) + ($('[data-role=meta]:visible').outerHeight(true) ?? 0) + ($('[data-role=toolbar]:visible').outerHeight(true) ?? 0) + ($('[data-role=pagination]:visible').outerHeight(true) ?? 0) + ($('[data-role=submit]:visible').outerHeight(true) ?? 0));
    });
  ...
  $('body').on('hidden.bs.collapse', '[data-role=description]', function(e) {
      $('[data-role=table]').animate({
        height: $(window).outerHeight(true) - (($('[data-role=header]:visible').outerHeight(true) ?? 0) + ($('[data-role=breadcrumb]:visible').outerHeight(true) ?? 0) + ($('[data-role=meta]:visible').outerHeight(true) ?? 0) + ($('[data-role=toolbar]:visible').outerHeight(true) ?? 0) + ($('[data-role=pagination]:visible').outerHeight(true) ?? 0))
      }, {
        complete: function() {
    ...
    $.ajax({
      ...
      .done(function(response) {
        if (typeof response.suggestions !== 'undefined') {
          let target = $(this).closest('form').find('select[name=' + response.selector + ']');
          target.empty();
   ...
   * Check all rows
   */
  $('body').on('change', '[data-role=checker]', function(e) {
  ...
  $('body').on('click', '.captcha-refresh', function(e) {
    ...
    let container = $(this);
    $.ajax({
      url: config.base_url + 'xhr/captcha',
      method: 'POST',
      beforeSend: function() {
