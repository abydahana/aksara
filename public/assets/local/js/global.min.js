	...
	$('body').on('change.fetch-parameter', '.fetch-parameter', function(e)
	{
		...
		.done(function(response)
		{
			if(response.url)
			{
				...
				.done(function(response)
				{
					if(typeof response.results !== 'undefined' && response.results.form_data !== 'undefined')
					{
						$.each(response.results.form_data, function(key, val)
						{
							$('<div class="col-sm-6"><label class="d-block"><input type="checkbox" name="parameter[' + key + ']"  value="1"' + (val.required ? ' checked disabled' : '') + '/> ' + val.label + (val.required ? ' <span class="text-danger text-sm">(' + (phrase.required ? phrase.required : 'Required') + ')</span>' : '') + '</label></div>').appendTo('.data-appended')
						})
					}
					else
					{
