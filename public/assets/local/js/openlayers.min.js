...
var openlayers										= (function()
{
	return {
		...
		render: function(_this_)
		{
			...
			else
			{
				...
				map.on('singleclick', function(event)
				{
					...
					if($('.route-start').length && !$('.route-start').val())
					{
						$('.route-start').val(ol.proj.transform(event.coordinate, map.getView().getProjection(), projection))
					}
					else if($('.route-end').length && !$('.route-end').val())
					{
						$('.route-end').val(ol.proj.transform(event.coordinate, map.getView().getProjection(), projection))
					}
				})
			}
		...
		unzip: function(source, spinner, ignore_draggable_marker)
		{
						...
						Object.keys(zip.files).forEach(function(filename)
						{
							if('geojson' == filename.split('.').pop().toLowerCase())
							{
								...
								zip.files[filename].async('string').then(function(blob)
								{
									...
									openlayers.geojson(source, spinner, ignore_draggable_marker)
								})
							}
						})
					})
			...
			({
				...
				style: function(feature, resolution)
				{
					...
					({
						...
						({
							scale: (feature.get('icon-scale') ? feature.get('icon-scale') : (icon_pattern ? .8 : 1)),
		 ...
		 * render from GeoJSON
		 */
		geojson: function(source, spinner, ignore_draggable_marker)
		{
			$('.preloader').remove(),
		...
		route: function(starting_point, destination, type)
		{
			...
			$.get('//router.project-osrm.org/route/v1/' + (type ? type : 'driving') + '/' + ($.isArray(starting_point) ? starting_point.join() : starting_point) + ';' + ($.isArray(destination) ? destination.join() : destination), function(response)
			{
