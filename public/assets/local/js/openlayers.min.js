	...
	return {
		...
		keyhole: function(source, spinner, ignore_draggable_marker, ignore_fit)
		{
			...
			map.on('singleclick', function(event)
			{
				...
				if(typeof target === 'undefined' || typeof target.point === 'undefined' || !target.point.get('name')) return;
				
				selected.clear(),
				
				selected.push(target.point),
				
				popup.show(event.coordinate, ('<div class="popup-placeholder"><div class="popup-content p-3"><label class="font-weight-bold">' + target.point.get('name') + '</label><hr class="row" />' + target.point.get('description') + '</div></div>'))
			})
		},
