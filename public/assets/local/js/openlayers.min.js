...
const openlayers = (function() {
  return {
    render: function(_this) {
      ...
      // Set coordinate
      (context.data('coordinate') && $.inArray(context.data('drawing-type'), ['point', 'coordinate']) !== -1 ? openlayers.coordinate((1 == context.data('draggable') ? true : false)) : null);
    ...
    geojsonString: function(shapes, type) {
      ...
      if (type && ['polygon', 'linestring'].includes(type)) {
        ...
        let select = new ol.interaction.Select({
          ...
          style: new ol.style.Style({
            stroke: new ol.style.Stroke({
              color: hex2rgba('#0099ff', 1),
              width: (stroke_width ? stroke_width : 2)
            }),
            fill: new ol.style.Fill({
              color: hex2rgba('#0099ff', .05)
            })
          })
        });
        ...
        select.on('select', function(event) {
          if (event.selected.length) {
            ...
            // Show popup
            popup.show(clickedCoordinate, `<div class="popup-content"><div class="list-group list-group-flush"><a href="javascript:void(0)" class="list-group-item list-group-item-action" onclick="removeFeature()"><i class="mdi mdi-trash-can-outline"></i> ${phrase('Remove feature')}</a></div></div>`);

            // Animate popup to center
            setTimeout(function() {
              map.getView().animate({
      ...
      mvtTile = new ol.layer.VectorTile({
        ...
        style: function(feature) {
          ...
          try {
            ...
            let fillOp = (type === 'Polygon' || type === 'MultiPolygon') ? Math.min(1, opacity) : 0.2;
            rgbaFill = `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, ${fillOp})`;
          } catch (e) {
            rgbaFill = `rgba(255, 0, 0, 0.2)`;
          }

          let rgbaStroke;
          try {
            let sRgb = ol.color.asArray(sColor);
            rgbaStroke = `rgba(${sRgb[0]}, ${sRgb[1]}, ${sRgb[2]}, 1)`;
          } catch (e) {
            rgbaStroke = sColor;
          }
    ...
    coordinate: function(draggable) {
      ...
      if (draggable) {
        ...
        drag.on('translateend', function(event) {
          ...
          $(apply_coordinate).val(geojsonFormat.writeFeatures(draggableMarker.getSource().getFeatures(), {
            featureProjection: map.getView().getProjection()
          }));
            ...
            if (draggableMarker) {
              ...
              try {
                ...
                if (vp) {
                  ...
                  map.getView().fit(extent, { size: map.getSize(), maxZoom: maxZoom });
                } else {
        ...
        .done(function(response) {
          ...
          $.each(response.steps, function(key, val) {
            ...
            if ((key + 1) >= response.steps.length && typeof OverlayScrollbarsGlobal !== 'undefined') {
              if ($('.find-route-results').attr('data-overlayscrollbars-initialize') === undefined) {
                ...
                OverlayScrollbarsGlobal.OverlayScrollbars($('.find-route-results')[0], {
                  scrollbars: {
                    theme: 'os-theme-dark',
                    autoHide: 'leave',
                    autoHideDelay: 500,
