...
var openlayers										= (function()
{
	return {
		...
		render: function(_this_)
		{
			...
			map.on('moveend', function(event)
			{
				...
				if(_this.attr('data-clustering-url') && !selected.length)
				{
					...
					try
					{
						checked						= JSON.parse(sessionStorage.getItem('checked_layers'))
					}
					catch(e)
					{
					}
		...
		{
			if('geojson' == source.split('.').pop().toLowerCase() || 'json' == source.split('.').pop().toLowerCase())
			{
				openlayers.geojson(source)
			}
			else if('zip' == source.split('.').pop().toLowerCase() || 'kmz' == source.split('.').pop().toLowerCase())
			{
		...
		unzip: function(source, spinner, ignore_draggable_marker, ignore_fit)
		{
			...
			{
				openlayers.geojson(sessionStorage.getItem(source.initial), spinner)
			}
							...
							Object.keys(zip.files).forEach(function(filename)
							{
								...
								else if('kml' == filename.split('.').pop().toLowerCase())
								{
									...
									zip.files[filename].async('string').then(function(blob)
									{
										...
										openlayers.keyhole(blobURL, title);
		...
		geojsonString: function(shapes, type)
		{
			...
			if(type)
			{
				...
				drawingManager.on('drawend', function(event)
				{
					...
					$(apply_latitude).val((typeof coordinate[1] !== 'undefined' ? coordinate[1] : 0)),
					$(apply_longitude).val((typeof coordinate[0] !== 'undefined' ? coordinate[0] : 0)),
				...
				modify.on('modifyend', function(event)
				{
					...
					if('google' == config.openlayers_search_provider)
					{
						...
						finder.geocode({location: coordinate}, function(response, status)
						{
							if(status === 'OK')
							{
								$(apply_address).val(response[0].formatted_address).trigger('change'),
		 ...
		 */
		geojson: function(source, spinner, ignore_draggable_marker, ignore_fit)
		{
			...
			map.on('singleclick', function(event)
			{
				...
				if(typeof target === 'undefined' || typeof target.point === 'undefined' || !target.point.get('object_id')) return;
				
				selected.clear(),
				
				selected.push(target.point),
				
				popup.show(event.coordinate, ('<div class="popup-placeholder"><div class="popup-content"></div></div>')),
				...
				({
					...
					data:
					{
						objectID: (typeof target !== 'undefined' ? target.point.get('object_id') : 0),
					...
					beforeSend: function()
					{
						...
						$('.popup-content').html('<div class="d-flex justify-content-center p-3"><div class="spinner-border spinner-border-sm" role="status"><span class="sr-only">' + (phrase.loading ? phrase.loading : 'Loading...') + '</span></div></div>')
					}
				})
				.done(function(response)
				{
					...
					(typeof response.content !== 'undefined' && response.content ? $('.popup-content, .identification-information').html(response.content) : popup.hide())
				})
			})
		},
		...
		keyhole: function(source, spinner, ignore_draggable_marker, ignore_fit)
		{
			...
			map.on('singleclick', function(event)
			{
				...
				if(typeof target === 'undefined' || typeof target.point === 'undefined') return;
				
				selected.clear(),
				
				selected.push(target.point),
				
				popup.show(event.coordinate, ('<div class="popup-placeholder"><div class="popup-content p-3">' + target.point.get('description') + '</div></div>'))
			})
		},
		...
		rest: function(source)
		{
			...
			map.on('singleclick', function(event)
			{
				...
				popup.show(event.coordinate, ('<div class="popup-placeholder"><div class="popup-content p-3"></div></div>')),
		...
		route: function(starting_point, destination, type)
		{
			...
			if('google' == config.openlayers_search_provider)
			{
				...
				routing.route({origin: ($.isArray(starting_point) ? starting_point.join() : starting_point), destination: ($.isArray(destination) ? destination.join() : destination), travelMode: (type == 'cycling' ? 'BICYCLING' : (type == 'walking' ? 'WALKING' : 'DRIVING'))}, function(response, status)
				{
					...
					{
						/* push way point to map */
						routeWayPoints.getSource().addFeature
						(
							new ol.Feature
							({
								type: 'place',
								geometry: new ol.geom.Point(ol.proj.fromLonLat(val.coordinate))
							})
						)
					}),
