				...
				if(typeof target !== 'undefined' && typeof target.point !== 'undefined')
				{
					...
					if(_this.attr('data-finder-url'))
					{
						...
						.done(function(response)
						{
							...
							else if(typeof target.point.values_ !== 'undefined')
							{
								console.log(target);
								/**
								...
								$.each(target.point.values_, function(key, val)
								{
									...
									content			+= '<div class="row text-sm' + (num ? ' border-top' : '') + '"><div class="col-4 text-muted text-right pr-0">' + key + '</div><div class="col-8 text-break-word">' + val + '</div></div>';
					...
					else
					{
						...
						$.get('https://nominatim.openstreetmap.org/reverse?accept-language=id&format=json&lat=' + coordinate.lat + '&lon=' + coordinate.lng + '&addressdetails=1', function(response)
						{
							if(typeof response !== 'undefined')
							{
								/* apply to inputs */
								$(apply_address).val(response.display_name).trigger('change'),
								$(apply_latitude).val(parseFloat(response.lat).toFixed(6) ),
								$(apply_longitude).val(parseFloat(response.lon).toFixed(6)),
