...
var openlayers										= (function()
{
	return {
		...
		render: function(_this_)
		{
			...
			map.on('moveend', function(event)
			{
				...
				if(_this.attr('data-clustering-url') && !selected.length)
				{
					...
					try
					{
						checked						= JSON.parse(localStorage.getItem('checked_layers'))
					}
					catch(e)
					{
					}
					...
					if(!previousCenter.length || distance > 100)
					{
						xhr							= $.ajax
						({
		...
		unzip: function(source, spinner, ignore_draggable_marker, ignore_fit)
		{
			...
			{
				openlayers.geojson(source, spinner, ignore_draggable_marker, ignore_fit)
			}
			else
			{
				var createZip						= '';
				...
				{
					createZip						+= String.fromCharCode(source.charCodeAt(i) & 0xFF);
				}
				
				var bytes							= new Uint8Array(createZip.length);
				
				for(var i = 0; i < createZip.length; i++)
				{
					bytes[i]						= createZip.charCodeAt(i);
				}
				
				/* write blob file */
				var blobURL							= URL.createObjectURL(new Blob([bytes], {type: 'application/zip'}));
		...
		geojson: function(source, spinner, ignore_draggable_marker, ignore_fit)
		{
			...
			map.on('singleclick', function(event)
			{
				...
				(typeof target !== 'undefined' ? selected.push(target.point) : null),
		...
		route: function(starting_point, destination, type)
		{
			...
			routeWayPoints							= new ol.layer.Vector
			({
				source: new ol.source.Vector(),
				style: new ol.style.Style
				({
			...
			routeLineString							= new ol.layer.Vector
			({
				source: new ol.source.Vector(),
				style: new ol.style.Style
				({
