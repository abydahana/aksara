	...
	return {
		...
		render: function(_this_)
		{
			...
			if(_this.attr('data-drawing-manager'))
			{
				...
				tileSource.setUrl('https://mt{0-3}.google.com/vt/lyrs=y&x={x}&y={y}&z={z}')
			}
			else
			{
				/* add default popup */
		...
		unzip: function(source, spinner, finder)
		{
			if(localStorage.getItem(source.initial) && !finder)
			{
						...
						Object.keys(zip.files).forEach(function(filename)
						{
							if('geojson' == filename.split('.').pop().toLowerCase())
							{
								...
								zip.files[filename].async('string').then(function(blob)
								{
									...
									openlayers.geojson(source, spinner, finder)
								})
							}
						})
					})
		...
		geojsonString: function(shapes, type)
		{
			...
			if(type)
			{
				...
				modify.on('modifyend', function(event)
				{
					...
					{
						coordinate					= ol.proj.transform(layer.getGeometry().flatCoordinates, 'EPSG:3857', 'EPSG:4326');
						coordinate					=
						{
							lat: coordinate[1],
							lng: coordinate[0]
		...
		geojson: function(source, spinner)
		{
			...
			{
				if(layerVector.getSource().getState() === 'ready' && !finder)
				{ 
		...
		arcgis: function(rest_url)
		{
			...
			map.on('singleclick', function(event)
			{
				...
				extent								= ol.proj.transformExtent([extent[0], extent[1], extent[2], extent[3]], 'EPSG:3857', 'EPSG:4326');
				...
				({
					...
					[
						...
						({
							...
							description: '<p class="mb-0">' + (phrase.draggable_on_drawing_mode ? phrase.draggable_on_drawing_mode : 'This can be drag on edit mode') + '</p>',
		...
		geocoder: function(_this)
		{
			...
			(
				'nominatim',
				{
					...
					countrycodes: 'id',
					placeholder: (phrase.search_a_place ? phrase.search_a_place : 'Search Place'),
					limit: 10,
					autoComplete: true,
					featureStyle: null
				}
			...
			if('google' == config.openlayers_search_provider)
			{
				...
				google.maps.event.addListener(autocomplete, 'place_changed', function()
				{
					...
					if(_this.attr('data-finder-url'))
					{
						...
						.done(function(response)
						{
							openlayers.unzip({url: URL.createObjectURL(response)}, null, true)
						})
					}
				})
			}
