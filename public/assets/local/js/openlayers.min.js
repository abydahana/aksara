  ...
  return {
    render: function(_this) {
      ...
      if (context.data('tiles-url') && context.data('tiles-url')) {
        ...
        tileSource = _isDarkMode()
          ? _makeTileSource(DARK_TILE_URL)
          : new ol.source.OSM();
      }
        ...
        map.on('singleclick', function(event) {
          ...
          window.loadOverlapPopup = function() {
            ...
            if (context.data('finder-url') && (typeof target === 'undefined' || typeof target.layer === 'undefined' || target.layer !== layerOverlap)) {
              const objectId = (typeof target !== 'undefined' && typeof target.point !== 'undefined' ? (target.point.get('objectId') || target.point.get('OBJECTID') || target.point.get('id') || (typeof target.point.getId === 'function' ? target.point.getId() : 0) || 0) : 0);
              ...
              drag.on('translateend', function(event) {
                if (typeof event.coordinate !== 'undefined') {
                  ...
                  $(applyLatitude).val((typeof coordinate[1] !== 'undefined' ? parseFloat(coordinate[1]).toFixed(6) : 0)),
                    ...
                    $(applyCoordinate).val(geojson.writeFeatures(event.features.getArray(), {
                      featureProjection: map.getView().getProjection()
                    }))
                }
              });
    ...
    geojsonString: function(shapes, type) {
      ...
      layerVector = new ol.layer.VectorImage({
        ...
        style: function(feature, resolution) {
          ...
          return [new ol.style.Style({
            ...
            fill: new ol.style.Fill({
              color: (pattern ? pattern : hex2rgba((fillColor ? fillColor : (feature.get('fill') ? feature.get('fill') : '#ff0000')), (iconScale ? parseFloat(iconScale) : (feature.get('fill-opacity') ? feature.get('fill-opacity') : .35))))
            }),
      ...
      if (type && ['polygon', 'linestring'].includes(type)) {
        function applyDrawFeatureData(feature, drawType, sourceFeatures) {
          ...
          $(applyCoordinate).val(geojson.writeFeatures(sourceFeatures, {
            featureProjection: map.getView().getProjection()
          }));
        ...
        require.js('https://maps.googleapis.com/maps/api/js?key=' + config.openlayersSearchKey + '&libraries=places&language=' + config.language, function() {
          ...
          google.maps.event.addListener(autocomplete, 'place_changed', function() {
            ...
            if (!drawingType) {
              ...
              let geojsonFormat = new ol.format.GeoJSON();
              $(applyCoordinate).val(geojsonFormat.writeFeatures([pointFeature], {
                featureProjection: map.getView().getProjection()
              }));
            }
    ...
    panorama: function() {
      ...
      require.js([
        config.baseUrl + 'assets/interactjs/interact.min.js',
        config.baseUrl + 'assets/openlayers/ol-street-view/ol-street-view.min.js'
      ], function() {
        ...
        map.getLayers().getArray().forEach(function(layer, index) {
          ...
          features.forEach(function(feature) {
            ...
            if (typeof feature.get('objectId') !== 'undefined' || typeof feature.get('id') !== 'undefined') {
              ...
              objectId = objectId.replace(/(<b[^>]+?>|<b>|<\/b>)/ig, ' ');
    ...
    download: function(type, pageFormat, dpi) {
      ...
      if ('png' == type) {
        ...
        if (canvas && typeof canvas[0] !== 'undefined') {
          require.js([
            config.baseUrl + 'assets/jspdf/jspdf.min.js'
          ], function() {
            ...
            // Central coordinate
            doc.text(
              phrase('Central') + ': ' + centerCoordinate[1].toFixed(4) + ',' + centerCoordinate[0].toFixed(4),
              margin,
              footerY
            );
...
function removeFeature() {
  ...
  $(applyCoordinate).val(geojson.writeFeatures(layerVector.getSource().getFeatures(), {
    featureProjection: map.getView().getProjection()
  }));
